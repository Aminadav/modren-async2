(function (global, factory) {
  ...
  async function asyncRoot(fct) {
    let errorHandler = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  ...
  class Queue {
    ...
    constructor(concurrency) {
      nanoassert(Number.isInteger(concurrency) || concurrency === Number.POSITIVE_INFINITY, 'concurrency must be an integer or positive infinity');
  ...
  async function reduce(iterable, reducer) {
    let initial = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
    nanoassert(typeof reducer === 'function', 'reducer must be a function');
  ...
  async function reduceRight(iterable, reducer) {
    let initial = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
    const arr = Array.from(iterable);
    arr.reverse();
  ...
  class Scheduler {
    ...
    constructor(fct, delay) {
      ...
      nanoassert(typeof this._startImmediate === 'boolean', 'startImmediate must be a boolean');
      this._maxPending = options.maxPending || 0;
      nanoassert(Number.isInteger(this._maxPending) || this._maxPending === Number.POSITIVE_INFINITY, 'maxPending must be an integer or positive infinity');
